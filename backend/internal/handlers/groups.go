	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if isMember.Status == "requested" && len(isMember.InvitedBy) > 0 && (p.Action == "accept_invite" || p.Action == "decline_invite") {
			...
			case p.Action == "accept_invite":
				...
				err = helpers.AddUserToGroupChat(r.Context(), app.DB, tx, groupID, userId)
...
func RespondRequest(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		case "approve":
			...
			err = helpers.AddUserToGroupChat(r.Context(), app.DB, tx, groupMem.GroupID, groupMem.UserID)
			if err != nil && !errors.Is(err, sql.ErrNoRows) {
...
func RemoveMember(app *app.App) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if (isCurrentMember || isOnwer) && isOnwer != isCurrentMember && getMember.Status == "joined" {
			...
			if err := helpers.RemoveUserFromGroupChat(r.Context(), app.DB, getMember.GroupID, getMember.UserID); err != nil {
